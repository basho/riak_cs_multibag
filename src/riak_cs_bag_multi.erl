...
-spec pool_name(pool_type(),
                undefined | riakc_obj:riakc_obj() | lfs_manifest() | cs_bucket()) ->
                       atom().
%% TODO: Use Macro for record name (after repo-spilitting)
pool_name(request_pool = PoolType, Manifest) when is_record(Manifest, lfs_manifest_v3) ->
